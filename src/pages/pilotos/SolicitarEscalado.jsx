...
export default function SolicitarEscalado() {
  ...
  if (!['APROBADO', 'VIGENTE'].includes(piloto.estado)) {
    return <div style={{ padding: 24, color: '#dc2626' }}>El piloto no está en estado válido para solicitar escalado.</div>;
  }

  const handleEnviar = () => {
    if (!adjunto) { setError('El adjunto es obligatorio para solicitar escalado.'); return; }
    crearSolicitud({ pilotoCodigo: codigo, tipo: 'ESCALADO', adjuntoNombre: adjunto }, usuario);
  ...
  return (
    <div style={{ maxWidth: 600 }}>
      ...
      <h2 style={h2}>Solicitar Escalado</h2>
        ...
        {tienePendiente ? (
          ...
          <>
            <label style={{ fontSize: 13, fontWeight: 600, color: '#374151', display: 'block', marginBottom: 4 }}>
              Acta / Correo de aprobación de escalado *
            </label>
            <FileInput value={adjunto} onChange={setAdjunto} />
            ...
            <div style={{ background: '#fef9c3', border: '1px solid #fde047', borderRadius: 6, padding: '8px 12px', marginTop: 12, fontSize: 12, color: '#854d0e' }}>
              ℹ️ Esta solicitud no cambia el estado del piloto. Gobierno deberá confirmarla para efectuar el escalado.
            </div>

            <div style={{ display: 'flex', gap: 10, marginTop: 16 }}>
              <button onClick={handleEnviar} style={btnPrimary}>Enviar solicitud de escalado</button>
...
const btnPrimary = { background: '#0284c7', color: 'white', border: 'none', borderRadius: 8, padding: '9px 20px', cursor: 'pointer', fontWeight: 600, fontSize: 14 };
